...
`;
    ...
    while (lines.length > 0) {
        ...
        document.querySelector("#formula-presets").appendChild(option);
    }
}
function updateFromGUI() {
    let defines = {};
    let RADIUS_OF_EARTH = document.querySelector("#RADIUS_OF_EARTH").value;
    try {
        defines["RADIUS_OF_EARTH"] = parseFloat(RADIUS_OF_EARTH).toExponential();
    }
    catch (e) { }
    if (document.querySelector("#XY_OFFSET").checked) {
        defines["XY_OFFSET"] = true;
    }
    terrainMat = makeTerrainMaterial(defines);
    trySetFormula(document.querySelector("#formula").value);
}
document.querySelector("#formula-update").onclick = evt => updateFromGUI();
document.querySelector("#formula-presets").onchange = evt => document.querySelector("#formula").value = document.querySelector("#formula-presets").value;
document.exitPointerLock = document.exitPointerLock || document['mozExitPointerLock'] || document['webkitExitPointerLock'] || (() => { });
function showFormulaGUI() {
    document.exitPointerLock();
    let gui = document.querySelector("#formulaGUI");
    gui.style.display = "block";
    let textarea = document.querySelector("#formula");
    textarea.value = terrainFormula;
    textarea.focus();
    textarea.selectionStart = textarea.value.length;
}
function hideFormulaGUI() {
    document.querySelector("#formulaGUI").style.display = "none";
}
...
function makeTerrainMaterial(defines) {
...
function tickPlayer(deltaTime) {
    let velocity = new THREE.Vector2(0, 0);
    velocity.x += touchVelocity;
    if (heldKeys.has(VK.W))
        velocity.x += 1;
    if (heldKeys.has(VK.S))
        velocity.x -= 1;
    if (heldKeys.has(VK.A))
        velocity.y += 1;
    if (heldKeys.has(VK.D))
        velocity.y -= 1;
...
function addTouchGUI() {
    document.querySelector("#touchGUI").style.display = "block";
    document.querySelector("#formula-button").onclick = () => {
        showFormulaGUI();
    };
}
let touchX = null;
...
window.requestAnimationFrame(tick);
//========= Height
let renderedCells = new Set(); // Cells stored as strings in the format x,y:size
...
class LODTerrain {
    ...
    makeChildren() {
        if (this.size <= LODTerrain.MIN_SIZE)
            return false;
    ...
    setupUniforms() {
        ...
        let getRelativeResolution = (x, y) => {
            if (renderedCells.has(cellKey(Math.floor(x / 4), Math.floor(y / 4), this.size * 4)))
                return 2;
            if (renderedCells.has(cellKey(Math.floor(x / 2), Math.floor(y / 2), this.size * 2)))
                return 1;
            return 0;
        };
        // Store them in the vector4 as [right, bottom, left, top]
        this.mesh.material.uniforms.lodEdges.value = new THREE.Vector4(getRelativeResolution(this.tilePos[0] + 1, this.tilePos[1]), getRelativeResolution(this.tilePos[0], this.tilePos[1] - 1), getRelativeResolution(this.tilePos[0] - 1, this.tilePos[1]), getRelativeResolution(this.tilePos[0], this.tilePos[1] + 1));
    }
    shouldFree(focus) {
...
class Patch {
    ...
    static fromSource(x, y, w, h, xRes, yRes) {
...
function makeTerrainGeometry(startX, startY, w, h, xRes, yRes) {
